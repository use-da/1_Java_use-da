...
public class ArrayExample {
	...
	//자료형 배열명[]=new자료형[배열크기];
	//int[] arr=new int[4];
	//int[]을 참조하는 변수 arr      -Heap영역에 int형 변수4개짜리 배열 할당(생성)
	//int[]시작 주소를 저장
	...
	public void ex2() {
		...
		arr=new int[4]; //Heap영역에 int 4칸짜리 배열을 할당(생성)하고 
		            //할당된 배열의 시작주소를 arr에 대입			
		//배열 길이(크기) : 4
		System.out.println("배열길이 : "+arr.length);
		//ar.length : arr이 참조하고 있는 배열의 길이
		...
		//stack영역은 컴파일러가 알아서 초기화X
		//heap영역은 컴파일러가 알아서 초기화O
		...
		//배열 초기화
		//1)인덱스를 이용한 초기화
		arr[0]=100; //arr int[]참조형 변수가 참조하고있는 
		            //0번 인덱스에 100을 대입
		arr[1]=200;
		arr[2]=300;
		arr[3]=400;
		
		//위에 바꾸면 for문도 수정해야해서 3대신 
	...
	//int10개를 저장하는 배열의 각 요소(index)에
	//10,20,30~100까지 대입
	
	int[]arr2= new int[10];  //index0~9
...
//		System.out.println(heightArr); //배열의 주소가 출력됨->heightArr참조 변수에는
		                               //배열의 시작주소가 저장되어 있기 때문에
		
		//Arrays.toString(배열명) : 배열에 저장된 모든 값을 한 줄로 같단히 출력, 단순 확인용
//		System.out.println(Arrays.toString(heightArr)); 
		
		System.out.println();
		
		double sum=0;
		System.out.println("입력받은 키 : ");
	...
	public void ex5() {
		...
		int[] arr= {10,20,30,40};   
		//int[]을 참조하는 변수arr 선언
		//int형 4칸짜리 배열을 heap영역에 할당(생성) 후
		//각각의 인덱스를 10,20,30,40으로 초기화
		//할당된 배열의 시작 주소를 arr변수에 대입
	...
	public void ex6() {
		String[]menuArr= {"김밥+라면","KFC","맘스터치","서브웨이",
				           "백반","순대국","파스타","삼겹살","빵"};
		//배열 index범위 내 난수 발생
		int ran=(int)(Math.random()*menuArr.length);   //0.0<=x<=1.0
	...
	//인원 수 만큼 점수를 입력 받아 합계, 평균, 최고점, 최저점을 출력
      // 입력 받을 인원 수 : 4
      // 1번 점수 입력 : 100
      // 2번 점수 입력 : 80
      // 3번 점수 입력 : 50
	...
	public void ex7() {
		...
		for(int i=0;i<scoreArr.length;i++) {
			...
			sum+=scoreArr[i]; //점수를 입력받자마자 sum에 누적	
		}
		//인덱스를 순서대로 최고,최저비교
		int max=scoreArr[0];
		int min=scoreArr[0];
	...
	//로또 번호 생성기
	//1~45 사이의 중복되지 않은 난수 6개
	//생성된 난수를 오름 차순 정렬
	
	public void createLottoNumber() {
		System.out.println("****로또번호 생성기****");
		
		//난수 6개 저장할 배열 선언 및 할당
		int[] lotto=new int[6];
		...
		for(int i=0;i<lotto.length;i++) {
			...
			for(int x=0; x<i; x++) {  
				if(lotto[x]==ran) {
					i--; //i값을 인위적으로 감소시켜 바깥쪽 for문의 증감식 i++ 가 실행 되었을 때
					     //i값이 현재 값을 유지하도록 만든다.
						 //(화살표를 왼쪽 1칸 + 오른쪽 1칸==제자리)
					break; //추가 검사가 불필요하기 때문에 멈춘다
				}
	...
	public void ex9() {
		...
		int[] copyArr=arr;  //arr변수에 저장된 배열의 시작 주소를 copyArr에 대입(얕은 복사)
	...
	public void ex10() {
		...
		//1)for문을 이용(index가 동일하다는 특징을 이용)
...
//		System.arraycopy(arr, 0, copyArr, 0, arr.length);
	...
	public void ex11() {
		...
		//arr1==null -> false == arr참조 변수가 무언가를 참조하고 있다
		
		if(arr1!=null) {//arr1이 참조하는 배열이 있을 때만 수행
