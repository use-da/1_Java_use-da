...
public class Example {
	...
	public void ex1() {
		...
		System.out.print("입력 : ");
//		String input=br.readLine();  //==sc.nextLine();    클릭+shift+f2
		//public String readLine() throws IOException
		//readLine() 메서드는 IOException을 던질 가능성이 있기 때문에 호출 시 반드시 예외처리를 하시오.
		//->Checked Exception
		...
		//코드 작성 시에는 예외처리를 반드시 하라고 컴파일 에러가 발생하지 않음
	...
	public void ex2() {
		...
		BufferedReader br= new BufferedReader(new InputStreamReader(System.in));
			...
			try {  //try : 예외가 발생할 것 같은 구문을 내부에 작성 
				...
				//catch : try구문에서 예외가 던져진 경우 매개변수(참조형)를 이용해 해당 객체를 잡음
	...
	public void ex3() {
		...
		try {
			...
			//Unreachable catch block for ArithmeticException. It is already handled by the catch block for Exception
											...
											//catch문이 여러개면 위에서부터 순서대로 발생한 예외를
											//검사하여 알맞은 매개변수를 가진 catch에서 처리한다
		...
		}finally {  //finally : catch 구문 수행 여부에 관계 없이 무조건 실행하는 구문
		...
		//예외 발생 == throw new 예외 클래스(); == 예외 객체가 생성되어 던저짐
		
		//catch(부모타입 예외 참조변수){}
		//->try에서 던저진 예외 객체가 catch구문 매개변수의 자식 타입이면 
		//부모타입 예외 참조변수=던져진 자식 객체->다형성(업캐스팅)이 적용되어 해당 catch문에 처리된다
	}
	...
	public void ex4() {
		...
		try {
			...
			e.printStackTrace();
			//e : 예외 객체 참조 변수
			//printStackTrace : 예외가 발생한 메서드의 순서를 출력
		}  
	...
	public void ex6() throws IOException {
		//해당 메서드에서 발생하는 IOException을 호출한 메서드로 던져버림
