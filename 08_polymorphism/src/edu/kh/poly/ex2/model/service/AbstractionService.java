...
import edu.kh.poly.ex2.model.vo.LYHCalculator;
...
public class AbstractionService {
	...
	public void ex1() {
		...
		//Animal을 상속 받은 자식 객체를 이용해 객체 생성
		Fish f1=new Fish();
		f1.setType("어류");    //자식 객체 내부에 부모 부분 확인
		//f1.breath();
		
		Person p1= new Person();
		p1.setType("포유류");  //자식 객체 내부에 부모 부분 확인
	
		
		//추상 클래스는 부모 타입의 참조 변수로 사용할 수 있다->업캐스팅
		...
		//객체배열 + 추상 클래스
		Animal[]arr=new Animal[3]; //Aniaml 참조변수 3개묶음
		...
		//arr[i]인덱스에 어떤 Animal 자식 객체가 있는지 모르지만 다들 강제 오버라이딩한
		//breath()라는 공통적인 메서드를 가지고 있으므로 arr[i].breath;작성 가능
		//대입되는 객체가 바뀌어도 코드 수정없이 사용가능
	...
	public void ex2() {
		//인터페이스는 미완성 메서드로만 이루어져 있어 직접 객체 생성 불가
		//KH k1=new KH();
		...
		sh1.lesson();
		gd1.lesson();
		//클래스나 추상클래스를 상속한 자식 객체는 내부에 부모 객체를 포함하고 있으나
		//인터페이스를 상속한 자식객체는 부모객체를 포함하지 않는다
		...
		for(int i=0;i<arr.length;i++) {
			...
			//동적 바인딩 : arr[0]->HSH.lesson();  arr[1]->HGD.lesson();	
	...
	public void ex3() {
		Calculator cal=new LYHCalculator();
