...
public class Example1 {
	//다형성 : 상속을 이용한 기술로 부모 타입의 참조 변수 하나가 여러 타입의 자식 객체를 참조할 수 있음
...
//		c2.setWeight(1.5);
		...
		//			 Car toString이 아니라 Truck toString에 연결
	...
	public void ex2() {
		...
		//업캐스팅 +동적 바인딩 장점
		//->코드 재사용성 증가, 코드 길이 감소
		printCar(arr[0]);   /*Car주소*/
		printCar(arr[1]);   /*Truck주소*/
		printCar(arr[2]);   /*Spark주소*/
...
//		((Truck)c3).loading();
		...
		//코드상의 오류는 발생x
		//실행시 ClassCastException : 형변환 예외 ->다운 캐스팅이 잘못된 경우 발생
		
		//*해결방법 : instanceof연산자로 다운캐스팅 할 타입이 맞는지 검사한다
	...
	public void ex4() {
		...
		if(c instanceof Car) {
			//c가 참조하는 객체는 Spark이지만 업캐스팅 상태이기 때문에 Car로도 인식됨
			System.out.println("부모 Car타입입니다.");
		}else {
			((Spark)c).dc();
		}
	}
	...
	public void check(Car c) {
		...
		if(c instanceof Spark) {
			...
			System.out.println("Car는 고유기능이 없습니다.");
		}
